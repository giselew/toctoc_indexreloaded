...
In a **4th step, the folding step**, CSS can be split up in CSS which loads **above the <head>-tag and CSS that will load after page load**. 
CSS is organised in files, it is possible to inline above CSS.
...
The API-key is valid for a given time and allows a limited number of calls. Free API-keys are available.
